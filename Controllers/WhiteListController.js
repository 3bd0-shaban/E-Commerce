...
export const Add_To_WhiteList = asyncHandler(async (req, res, next) => {
    ...
    const whitelist = await Users.findByIdAndUpdate(
        req.user._id,
        {
            $push: {
                whiteList: productId,
    ...
    return res.json({ message: "Product added to white list" });
});
export const Fetch_Product_In_WhiteList = asyncHandler(
    async (req, res, next) => {
        const whiteList = await Users.findOne(req.user._id)
            .select("whiteList")
            .populate("whiteList._id", "name images des price");
        if (!whiteList)
            return next(
                new ErrorHandler("you do not have any products in white list", 400)
            );
        return res.json(whiteList);
    }
);
export const Delete_All_User_Whitelist = asyncHandler(
    async (req, res, next) => {
        const user = await Users.findOne(req.user._id);
...
export const Delete_User_Product_Whitelist = asyncHandler(
    async (req, res, next) => {
        const productId = await Products.findById(req.params.id);
